...
namespace FinnovationLabs.OpenBanking.Library.Connector.ObModel.PaymentInitiation.V3p1p1.Model
{
    ...
    public partial class OBMultiAuthorisation1 :  IEquatable<OBMultiAuthorisation1>, IValidatableObject
    {
        ...
        public OBMultiAuthorisation1(OBExternalStatus2Code status = default(OBExternalStatus2Code), int? numberRequired = default(int?), int? numberReceived = default(int?), DateTime? lastUpdateDateTime = default(DateTime?), DateTime? expirationDateTime = default(DateTime?))
        {
            // to ensure "status" is required (not null)
            if (status == null)
            {
                throw new InvalidDataException("status is a required property for OBMultiAuthorisation1 and cannot be null");
            }
            else
            {
                this.Status = status;
