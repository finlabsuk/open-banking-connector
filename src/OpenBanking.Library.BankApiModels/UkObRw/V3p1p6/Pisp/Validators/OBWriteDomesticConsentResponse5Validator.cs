...
{
    public class
        OBWriteDomesticConsentResponse5Validator : AbstractValidator<
            OBWriteDomesticConsentResponse5>
    {
        public OBWriteDomesticConsentResponse5Validator()
        {
        ...
        private void CreateRules()
        {
            RuleFor(x => x)
                ...
                .Must(
                    ...
                    $"Invalid value for (nested) member of {typeof(OBWriteDomesticConsentResponse5).FullName}: " +
                    "{ExceptionMessage}");

            // Custom validation
            RuleFor(x => x.Data)
                .SetValidator(new OBWriteDomesticConsentResponse5DataValidator());
