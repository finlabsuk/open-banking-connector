...
{
    public class
        OBWriteDomestic2Validator : AbstractValidator<OBWriteDomestic2>
    {
        public OBWriteDomestic2Validator()
        {
        ...
        private void CreateRules()
        {
            RuleFor(x => x)
                ...
                .Must(
                    ...
                    $"Invalid value for (nested) member of {typeof(OBWriteDomestic2).FullName}: " +
                    "{ExceptionMessage}");

            // Custom validation
            RuleFor(x => x.Data)
                .SetValidator(new OBWriteDomestic2DataValidator());
