...
public enum DateTimeOffsetUnixConverterEnum
{
...
public abstract class
    DateTimeOffsetGenericUnixConverter<TDateTimeOffset> : JsonConverterWithOptions<TDateTimeOffset,
        DateTimeOffsetUnixConverterEnum>
{
    public DateTimeOffsetGenericUnixConverter(JsonConverterLabel? jsonConverterLabel) :
        base(jsonConverterLabel) { }

    protected long GetUnixTime(DateTimeOffset time, DateTimeOffsetUnixConverterEnum options)
    {
        long seconds = time.ToUnixTimeSeconds();
        long timeValue = options.HasFlag(DateTimeOffsetUnixConverterEnum.UnixMilliSecondsJsonFormat)
            ? seconds * 1000
            : seconds;
        return timeValue;
    }

    protected DateTimeOffset GetTime(long unixTime, DateTimeOffsetUnixConverterEnum options)
    {
        long seconds = options.HasFlag(DateTimeOffsetUnixConverterEnum.UnixMilliSecondsJsonFormat)
            ? unixTime / 1000
            : unixTime;
...
public class DateTimeOffsetUnixConverter : DateTimeOffsetGenericUnixConverter<DateTimeOffset>
{
    ...
    {
        DateTimeOffsetUnixConverterEnum options = GetOptions(serializer);
