        ...
        public static IHostBuilder AddGenericHostConfiguration(this IHostBuilder hostBuilder, string[] args) =>
            hostBuilder.ConfigureAppConfiguration(
                (hostingContext, config) =>
                {
                    ...
                    // // Setting extraction without using class - removed to allow for class defaults, validation
                    // string useUserSecretsKey =
                    //     new ConfigurationSourcesSettings().SettingsGroupName + ":" +
                    //     nameof(ConfigurationSourcesSettings.UseUserSecrets);
                    // var useUserSecrets = defaultConfig.GetValue<bool>(useUserSecretsKey);
                    // string awsSsmParameterPrefixKey =
                    //     new ConfigurationSourcesSettings().SettingsGroupName + ":" +
                    //     nameof(ConfigurationSourcesSettings.AwsSsmParameterPrefix);
                    // string awsSsmParameterPrefix =
                    //     defaultConfig[awsSsmParameterPrefixKey] ?? string.Empty;

                    ConfigurationSourcesSettings configurationSourcesSettings =
                        defaultConfig
                            .GetSection(new ConfigurationSourcesSettings().SettingsGroupName)
                            .Get<ConfigurationSourcesSettings>() ?? new ConfigurationSourcesSettings();
                    configurationSourcesSettings.Validate();
