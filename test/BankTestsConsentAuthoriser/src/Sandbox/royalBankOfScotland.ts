...
export const consentUIInteractions: ConsentUIInteractions = async (page, navigationPromise, consentVariety, bankUser) => {
    ...
    await page.waitForSelector('#account-list > .row-element:nth-child(2) > dl > .action > button')
    await page.click('#account-list > .row-element:nth-child(2) > dl > .action > button')
